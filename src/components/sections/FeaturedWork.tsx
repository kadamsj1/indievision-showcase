    ...
    <section className="py-16 sm:py-24 md:py-32 bg-background">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-6 mb-10 sm:mb-16">
          ...
          <Link
            to="/portfolio"
            className="group flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors py-2"
          >
            View All Projects
            <ArrowUpRight
              size={18}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {featuredProjects.map((project, index) => (
            <Link
              key={project.id}
              to={`/portfolio`}
              className="group relative aspect-[4/5] sm:aspect-[4/5] overflow-hidden rounded-sm hover-lift"
            ...
            >
              ...
              <div className="absolute inset-0 p-5 sm:p-6 flex flex-col justify-end">
                <span className="text-primary text-xs sm:text-sm tracking-wide mb-1 sm:mb-2">
                  {project.category}
                </span>
                <h3 className="font-serif text-xl sm:text-2xl text-foreground mb-1 group-hover:text-primary transition-colors">
                  {project.title}
                </h3>
                ...
                {/* Arrow Icon */}
                <div className="absolute top-4 sm:top-6 right-4 sm:right-6 w-9 h-9 sm:w-10 sm:h-10 rounded-full border border-foreground/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 group-hover:bg-primary group-hover:border-primary">
                  <ArrowUpRight size={16} className="text-foreground group-hover:text-primary-foreground" />
                </div>
              </div>
            </Link>
          ))}
